...
object KafkaDirectWordCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var kafkaStream: InputDStream[(String, String)] = null //两种情况
    ...
    if (children > 0) {
      ...
      //这个会将kafka的消息进行transform，最终 kafak 的数据都会变成(kafka的key, message)这样的 tuple
    ...
    //依次迭代DStream中的RDD //每隔一段时间生成一个RDD
    messages.foreachRDD { rdd =>
      //对RDD进行操作，触发Action
      rdd.foreachPartition(partition =>
        partition.foreach(x => { //一个分区里面的多条数据
          println(x)  //打印rdd
        })
      )

      //把执行完毕后分区的偏移量 offsetRanges 更新到zookeeper
      for (o <- offsetRanges) {
        //  第一次：/g001/offsets/wordcount/0
        val zkPath = s"${topicDirs.consumerOffsetDir}/${o.partition}"
        //将该 partition 的 offset 保存到 zookeeper
        //  更新后：/g001/offsets/wordcount/0/20000
