...
object IpLocation2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    reduced.foreach(tp => {
      ...
      val conn: Connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/bigdata?charatorEncoding=utf-8", "root", "123456")
      //写入大量数据的时候，有没有问题？
     使用的连接太多了 写一条拿出一个连接  写一条拿一个连接  很昂贵  哪怕是连接池也不好
    ...
    val conn: Connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/bigdata_ip?characterEncoding=UTF-8", "root", "123456")
